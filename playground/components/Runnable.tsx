...
function formatObjectLike(
  ...
  return (
    <>
      <span className={style.code}>
        <span className={style.classNameIdent}>
...
function formatValue(obj: unknown, nestIdent = 0): JSX.Element {
  if (typeof obj === "string")
    return (
      <span className={`${style.code} ${style.string}`}>
        "
        {obj
          .replace(/&/g, "&amp;")
          .replace(/</g, "&lt;")
    ...
    return <span className={`${style.code} ${style.number}`}>{obj}</span>
  else if (typeof obj === "function")
    return <span className={style.code}>[Function]</span>
  else if (obj === undefined)
    return <span className={`${style.code} ${style.keyword}`}>undefined</span>
  else if (obj === null)
    return <span className={`${style.code} ${style.keyword}`}>null</span>
  else if (obj === true)
    return <span className={`${style.code} ${style.keyword}`}>true</span>
  else if (obj === false)
    return <span className={`${style.code} ${style.keyword}`}>false</span>
    ...
    return (
      <span className={style.code}>
        <span className={style.classNameIdent}>Set </span>({obj.size}){" {"}
...
export function Runnable(props: { code: string }) {
  ...
  const recalcHeight = () => {
    ...
    if (!current) return
    setNewHeight(
      `${window.innerHeight - topPaneRef.current.clientHeight - 55 * 3}px`,
    )
  }
  ...
  const specialConsole = {
    log: (...messages: any[]) => {
      setLogs([
        ...logs,
        ...messages.map((msg) => ({ kind: LogKind.Log, message: msg })),
      ])
    },
    warn: (...messages: any[]) => {
      setLogs([
        ...logs,
        ...messages.map((msg) => ({ kind: LogKind.Warn, message: msg })),
      ])
    },
    error: (...messages: any[]) => {
      setLogs([
        ...logs,
        ...messages.map((msg) => ({ kind: LogKind.Error, message: msg })),
      ])
    },
  }
  ...
  return (
    ...
    >
      ...
      <div className={style.runSection}>
        ...
        <button className={style.button} onClick={() => setLogs([])}>
          Clear
        </button>
        <br />
        <div
          className={style.runSectionResult}
