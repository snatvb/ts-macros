...
export const binaryNumberActions: Record<
  number,
  (left: number, right: number) => ts.Expression
> = {
  [ts.SyntaxKind.MinusToken]: (left: number, right: number) =>
    createNumberNode(left - right),
  [ts.SyntaxKind.AsteriskToken]: (left: number, right: number) =>
    createNumberNode(left * right),
  [ts.SyntaxKind.SlashToken]: (left: number, right: number) =>
    createNumberNode(left / right),
  [ts.SyntaxKind.LessThanToken]: (left: number, right: number) =>
    left < right ? ts.factory.createTrue() : ts.factory.createFalse(),
  [ts.SyntaxKind.LessThanEqualsToken]: (left: number, right: number) =>
    left <= right ? ts.factory.createTrue() : ts.factory.createFalse(),
  [ts.SyntaxKind.GreaterThanToken]: (left: number, right: number) =>
    left > right ? ts.factory.createTrue() : ts.factory.createFalse(),
  [ts.SyntaxKind.GreaterThanEqualsToken]: (left: number, right: number) =>
    left >= right ? ts.factory.createTrue() : ts.factory.createFalse(),
  [ts.SyntaxKind.AmpersandToken]: (left: number, right: number) =>
    createNumberNode(left & right),
  [ts.SyntaxKind.BarToken]: (left: number, right: number) =>
    createNumberNode(left | right),
  [ts.SyntaxKind.CaretToken]: (left: number, right: number) =>
    createNumberNode(left ^ right),
  [ts.SyntaxKind.PercentToken]: (left: number, right: number) =>
    createNumberNode(left % right),
}

export const binaryActions: Record<
  number,
  (
    origLeft: ts.Expression,
    origRight: ts.Expression,
    left: unknown,
    right: unknown,
...
export const possiblyUnknownValueBinaryActions: Record<
  number,
  (
    origLeft: ts.Expression,
    origRight: ts.Expression,
...
export const unaryActions: Record<
  number,
  (val: unknown) => ts.Expression | undefined
> = {
...
export const labelActions: Record<number, (statement: any) => ts.Expression> = {
  ...
  [ts.SyntaxKind.ForStatement]: (node: ts.ForStatement) => {
    ...
    return createObjectLiteral({
      ...
      initializer: createObjectLiteral({
        variables:
          variables && ts.factory.createArrayLiteralExpression(variables),
        expression,
      }),
  ...
  [ts.SyntaxKind.VariableStatement]: (node: ts.VariableStatement) => {
    ...
    return createObjectLiteral({
      ...
      declarationType: hasBit(node.declarationList.flags, ts.NodeFlags.Const)
        ? ts.factory.createStringLiteral("const")
        : hasBit(node.declarationList.flags, ts.NodeFlags.Let)
          ? ts.factory.createStringLiteral("let")
          : ts.factory.createStringLiteral("var"),
    })
  },
}
