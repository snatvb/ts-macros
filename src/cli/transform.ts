...
export function createFile(
  ...
  const withoutFilename = providedPath.slice(
    0,
...
export function pretranspile(
  settings: PretranspileSettings,
  ...
  if (settings.watch) {
    createMacroTransformerWatcher(
      config,
      {
        ...
        afterUpdate: settings.exec
          ? (isInitial) =>
              isInitial && childProcess.exec(settings.exec as string)
          : undefined,
      },
      settings.emitjs,
      transformerConfig,
    ...
    for (const file of program.getSourceFiles()) {
      ...
      createFile(
        path.join(
          process.cwd(),
          settings.dist,
          file.fileName.slice(process.cwd().length),
        ),
        settings.emitjs
          ? ts.transpile(transformed, program.getCompilerOptions())
          : transformed,
        settings.emitjs,
      )
    }
