...
export default {
  $$loadEnv: {
    call: (args, transformer, callSite) => {
      const extraPath =
        args.length && ts.isStringLiteral(args[0]) ? args[0].text : ""
      let dotenv
      try {
      ...
      return transformer.context.factory.createCallExpression(
        ...
        extraPath
          ? [
              transformer.context.factory.createObjectLiteralExpression([
  ...
  $$define: {
    call: ([name, value, useLet, exportDecl], transformer, callSite) => {
      ...
      const list = transformer.context.factory.createVariableDeclarationList(
        ...
        transformer.getBoolFromNode(useLet)
          ? ts.NodeFlags.Let
          : ts.NodeFlags.Const,
      )
  ...
  $$err: {
    call: ([msg], transformer, callSite) => {
      ...
      throw new MacroError(
        callSite,
        `${lastMacro ? `In macro ${lastMacro.macro.name}: ` : ""}${strVal}`,
      )
    },
  },
  $$includes: {
    call: ([array, item], transformer, callSite) => {
      ...
      if (strContent) {
        ...
        const normalArr = array.elements.map((el) =>
          transformer.getLiteralFromNode(transformer.expectExpression(el)),
        )
  ...
  $$ts: {
    call: ([code], transformer, callSite) => {
      const str = transformer.getStringFromNode(
      ...
      const visitor = (node: ts.Node): ts.Node =>
        ts.factory.cloneNode(
          ts.visitEachChild(node, visitor, transformer.context),
        )
      return ts.visitNodes(
        result.statements,
        visitor,
      ) as unknown as Array<ts.Statement>
    },
  },
  $$escape: {
    call: ([code], transformer, callSite) => {
      ...
      if (ts.isBlock(maybeFn.body)) {
        const hygienicBody = [
          ...transformer.makeHygienic(
            maybeFn.body.statements as unknown as ts.Statement[],
          ),
        ]
  ...
  $$propsOfType: {
    call: (_args, transformer, callSite) => {
      ...
      return ts.factory.createArrayLiteralExpression(
        type
          .getProperties()
          .map((sym) => ts.factory.createStringLiteral(sym.name)),
      )
    },
  },
  $$typeToString: {
    call: ([simplifyType, nonNullType, fullExpand], transformer, callSite) => {
      ...
      return ts.factory.createStringLiteral(
        transformer.checker.typeToString(
          type,
          undefined,
          transformer.getBoolFromNode(fullExpand)
  ...
  $$typeMetadata: {
    call: ([collectProps, collectMethods], transformer, callSite) => {
      ...
      const stringifyType = (type: ts.Type) =>
        ts.factory.createStringLiteral(
          transformer.checker.typeToString(
            transformer.checker.getNonNullableType(type),
            undefined,
            ts.TypeFormatFlags.NoTruncation,
          ),
        )
        ...
        if (callSig && shouldCollectMethods) {
          methods.push(
            ts.factory.createObjectLiteralExpression([
              ...
              ts.factory.createPropertyAssignment(
                ...
                ts.factory.createObjectLiteralExpression(
                  ts
                    .getJSDocTags(valueDecl)
                    .map((tag) =>
                      ts.factory.createPropertyAssignment(
                        tag.tagName.text,
                        typeof tag.comment === "string"
                          ? ts.factory.createStringLiteral(tag.comment)
                          : ts.factory.createTrue(),
                      ),
                    ),
                ),
              ...
              ts.factory.createPropertyAssignment(
                ...
                ts.factory.createArrayLiteralExpression(
                  callSig.getParameters().map((method) => {
                    const paramType =
                      transformer.checker.getTypeOfSymbol(method)
                    return ts.factory.createObjectLiteralExpression([
                      ...
                      ts.factory.createPropertyAssignment(
                        "type",
                        stringifyType(paramType),
                      ),
                      ts.factory.createPropertyAssignment(
                        "optional",
                        hasBit(method.flags, ts.SymbolFlags.Optional)
                          ? ts.factory.createTrue()
                          : ts.factory.createFalse(),
                      ),
                    ])
              ...
              ts.factory.createPropertyAssignment(
                "returnType",
                stringifyType(callSig.getReturnType()),
              ),
            ]),
          )
          ...
          properties.push(
            ts.factory.createObjectLiteralExpression([
              ...
              ts.factory.createPropertyAssignment(
                "type",
                stringifyType(propType),
              ),
              ts.factory.createPropertyAssignment(
                "optional",
                hasBit(property.flags, ts.SymbolFlags.Optional)
                  ? ts.factory.createTrue()
                  : ts.factory.createFalse(),
              ),
            ]),
      ...
      return ts.factory.createObjectLiteralExpression([
        ts.factory.createPropertyAssignment(
          "name",
          ts.factory.createStringLiteral(type.symbol?.name || "anonymous"),
        ),
        ts.factory.createPropertyAssignment(
          "properties",
          ts.factory.createArrayLiteralExpression(properties),
        ),
        ts.factory.createPropertyAssignment(
          "methods",
          ts.factory.createArrayLiteralExpression(methods),
        ),
      ])
    },
  },
  ...
  $$map: {
    call: ([exp, visitor], transformer, callSite) => {
      ...
      const kindParamName =
        fn.parameters[1] &&
        ts.isIdentifier(fn.parameters[1].name) &&
        ...
        if ("body" in parent) {
          ...
          transformer.addComptimeSignature(
            ...
            fnBodyToString(
              transformer.checker,
              callableFn,
              transformer.context.getCompilerOptions(),
            ),
            signature.parameters.map((p) => p.name),
          )
          return
        }
      }
