...
export class MacroTransformer {
    ...
    for (const stmt of node.statements) {
      if (
        ...
        if (
          ...
          const filtered = stmt.importClause.namedBindings.elements.filter(
            (el) => {
              if (el.isTypeOnly) {
                return this.config.keepImports
              }
              const sym = resolveAliasedSymbol(
                this.checker,
                this.checker.getSymbolAtLocation(el.name),
              )
              if (!sym) {
                return true
              }
              if (this.macros.has(sym) || nativeMacros[sym.name]) {
                return false
              } else if (
                (hasBit(sym.flags, ts.SymbolFlags.Interface) &&
                  !hasBit(sym.flags, ts.SymbolFlags.Class)) ||
                hasBit(sym.flags, ts.SymbolFlags.ConstEnum) ||
                hasBit(sym.flags, ts.SymbolFlags.TypeAlias)
              ) {
                return this.config.keepImports
              } else {
                return true
            ...
            statements.push(
              ts.factory.updateImportDeclaration(
                ...
                ts.factory.createImportClause(
                  stmt.importClause.isTypeOnly,
                  undefined,
                  ts.factory.createNamedImports(filtered),
                ),
          ...
          const sym = resolveAliasedSymbol(
            this.checker,
            this.checker.getSymbolAtLocation(stmt.importClause.name),
          )
      ...
      const res = this.visitor(stmt) as
        | Array<ts.Statement>
        | ts.Statement
        | undefined
  ...
  expectExpression(node: ts.Node): ts.Expression {
    ...
    if (!visited || !ts.isExpression(node)) {
      throw new MacroError(node, "Expected an expression.")
    }
  ...
  expectStatement(node: ts.Node): ts.Statement {
    ...
    if (!visited || !ts.isStatement(visited)) {
      throw new MacroError(node, "Expected a statement.")
    }
  ...
  expect<T extends ts.Node = ts.Node>(node: T, kind: ts.SyntaxKind): T {
    ...
    if (!visited || visited.kind !== kind) {
      throw new MacroError(node, `Expected SyntaxKind ${kind}.`)
    }
    return visited as T
  }
  ...
  visitor(node: ts.Node): ts.VisitResult<ts.Node | undefined> {
    if (
      ...
      for (let i = 0; i < node.parameters.length; i++) {
        ...
        if (!ts.isIdentifier(param.name)) {
          throw new MacroError(
            param,
            "You cannot use deconstruction patterns in macros.",
          )
        }
      ...
      const macro = {
        ...
        body: node.body,
        typeParams:
          (node.typeParameters as unknown as Array<ts.TypeParameterDeclaration>) ||
          [],
        node,
        namespace: ts.isModuleBlock(node.parent)
          ? node.parent.parent
          : undefined,
      }
    ...
    if (ts.isLabeledStatement(node)) {
      ...
      if (ts.isLabeledStatement(statementNode)) {
        ...
        if (!labelRes) {
          ...
          const foundStmt = labelRes.findIndex(
            (node) => labelActions[node.kind],
          )
          if (foundStmt === -1) {
            return node
          }
      ...
      results.push(
        ...ts.visitEachChild(macro.body, this.boundVisitor, this.context)
          .statements,
      )
      const acc = macro.params.find(
        (p) => p.marker === MacroParamMarkers.Accumulator,
      )
      if (acc) {
        acc.defaultVal = ts.factory.createNumericLiteral(
          +(acc.defaultVal as ts.NumericLiteral).text + 1,
        )
      }
      this.macroStack.pop()
      return results
    ...
    if (ts.isExpressionStatement(node)) {
      if (
        ...
        const statements = this.runMacroFromCallExpression(
          node.expression,
        ...
        ts.isNonNullExpression(node.expression.tag)
      ) {
        const statements = this.runMacroFromTemplateExpression(
          node.expression,
    ...
    if (ts.canHaveDecorators(node) && ts.getDecorators(node)?.length) {
      ...
      for (let i = decorators.length - 1; i >= 0; i--) {
        ...
        if (
          ...
          ts.isNonNullExpression(decorator.expression.expression)
        ) {
          const res = this.runMacroFromCallExpression(
            decorator.expression,
            decorator.expression.expression.expression,
            prev || decorator.parent,
          )
          if (res && res.length) {
    ...
    if (
      ...
      ts.isNonNullExpression(node.tag)
    ) {
      const statements = this.runMacroFromTemplateExpression(
        node,
        node.tag.expression,
    ...
    if (this.macroStack.length) {
      ...
      if (
        ...
        if (
          ...
          if (ts.isObjectLiteralExpression(exp)) {
            const name = ts.isPropertyAccessExpression(node)
              ? getNameFromProperty(node.name)
              : this.getNumberFromNode(
                  this.expectExpression(node.argumentExpression),
                )
            if (!name) {
              return node
            }
            const prop = exp.properties.find(
              (p) => p.name && getNameFromProperty(p.name) === name,
            )
            if (!prop) {
              return ts.factory.createNull()
            }
        ...
        return ts.factory.createTemplateHead(
          node.text,
          node.rawText,
          node.templateFlags,
        )
      } else if (ts.isTemplateMiddle(node)) {
        return ts.factory.createTemplateMiddle(
          node.text,
          node.rawText,
          node.templateFlags,
        )
      } else if (ts.isTemplateTail(node)) {
        return ts.factory.createTemplateTail(
          node.text,
          node.rawText,
          node.templateFlags,
        )
      ...
      // Detects use of a macro parameter and replaces it with a literal
        ...
        for (const varNode of node.declarationList.declarations) {
          if (
            ts.isIdentifier(varNode.name) &&
            varNode.name.text.startsWith("$")
          ) {
            store.set(
              varNode.name.text,
              varNode.initializer
                ? this.expectExpression(varNode.initializer)
                : ts.factory.createIdentifier("undefined"),
            )
          } else {
            leftovers.push(
              this.expect(varNode, ts.SyntaxKind.VariableDeclaration),
            )
          }
        }
        if (leftovers.length) {
        ...
        node.elements.some((t) => ts.isSpreadElement(t))
      ) {
        const elements = []
      ...
      // Detects a ternary expression and tries to remove it if possible
        ...
        if (res === false) {
          ...
          return ts.factory.createConditionalExpression(
            param,
            undefined,
            this.expectExpression(node.whenTrue),
            undefined,
            this.expectExpression(node.whenFalse),
          )
        }
      }
      ...
      // Detects an if statement and tries to remove it if possible
        ...
        return ts.factory.createIfStatement(
          condition,
          this.expectStatement(node.thenStatement),
          node.elseStatement
            ? this.expectStatement(node.elseStatement)
            : undefined,
        )
      }
      ...
      // Detects a binary operation and tries to remove it if possible
        ...
        if (leftVal === NO_LIT_FOUND || rightVal === NO_LIT_FOUND) {
          return (
            possiblyUnknownValueBinaryActions[op]?.(
              left,
              right,
              leftVal,
              rightVal,
            ) ?? ts.factory.createBinaryExpression(left, op, right)
          )
        }
        if (
          binaryNumberActions[op] &&
          typeof leftVal === "number" &&
          typeof rightVal === "number"
        ) {
          return binaryNumberActions[op](leftVal, rightVal)
        } else {
          return (
            binaryActions[op]?.(left, right, leftVal, rightVal) ??
            ts.factory.createBinaryExpression(left, op, right)
          )
        }
      }
      ...
      // Detects a repetition
        ...
        node.expression.operator === ts.SyntaxKind.PlusToken &&
        ts.isArrayLiteralExpression(node.expression.operand)
      ) {
        return this.execRepetition(
          getRepetitionParams(this.checker, node.expression.operand),
        )
      } else if (ts.isPrefixUnaryExpression(node)) {
        if (
          node.operator === ts.SyntaxKind.PlusToken &&
          ts.isArrayLiteralExpression(node.operand)
        ) {
          const params = getRepetitionParams(this.checker, node.operand)
          if (!params.separator) {
            throw new MacroError(
              node,
              "Repetition separator must be included if a repetition is used as an expression.",
            )
          }
        ...
        const repNodeIndex = node.arguments.findIndex(
          (arg) =>
            ts.isPrefixUnaryExpression(arg) &&
            arg.operator === ts.SyntaxKind.PlusToken &&
            ts.isArrayLiteralExpression(arg.operand),
        )
        if (repNodeIndex !== -1) {
          const repNode = (
            node.arguments[repNodeIndex] as ts.PrefixUnaryExpression
          ).operand as ts.ArrayLiteralExpression
          ...
          if (!params.separator) {
            ...
            return ts.visitNode(
              ts.factory.createCallExpression(
                node.expression,
                node.typeArguments,
                finalArgs as Array<ts.Expression>,
              ),
              this.boundVisitor,
            )
          }
  ...
  execRepetition(
    { fn, literals, separator, indexTypes }: RepetitionData,
    wrapStatements?: boolean,
  ): Array<ts.Node> {
    const newBod: Array<ts.Node> = []
    const repeatNames = fn.parameters.map((p) => p.name.getText())
    const elementSlices: Array<Array<ts.Expression>> = Array.from(
      { length: repeatNames.length },
      () => [],
    )
    ...
    for (
      ;
      this.repeat[ind].index < totalLoopsNeeded;
      this.repeat[ind].index++
    ) {
      newBod.push(...this.transformFunction(fn, wrapStatements))
    }
    this.repeat.pop()
    return separator && separators[separator]
      ? [separators[separator](this, newBod)]
      : newBod
  }
    ...
    if ("statements" in fn.body) {
      if (wrapStatements) {
        newBod.push(
          wrapExpressions(
            fn.body.statements
              .map((node) => this.maybeStatement(node))
              .filter((el) => el) as ts.Statement[],
          ),
        )
      } else {
        for (const stmt of fn.body.statements) {
          ...
          if (res) {
            if (Array.isArray(res)) {
              newBod.push(...(res as ts.Statement[]))
            } else {
              newBod.push(res as ts.Statement)
            }
          }
    ...
    if (paramMacro.spread) {
      ...
      if (spreadItems.length === 1 && ts.isSpreadElement(spreadItems[0])) {
        ...
        return ts.factory.createArrayLiteralExpression(
          params.slice(paramMacro.start) as Array<ts.Expression>,
        )
      }
    }
  ...
  runMacroFromTemplateExpression(
    call: ts.TaggedTemplateExpression,
    name: ts.Expression,
  ): Array<ts.Statement> | undefined {
    const macro = this.macros.get(
      resolveAliasedSymbol(
        this.checker,
    ...
    const strings: ts.StringLiteral[] = [
        ts.factory.createStringLiteral(call.template.head.text),
      ],
    ...
    return this.execMacro(
      macro,
      ts.factory.createNodeArray([
        ts.factory.createArrayLiteralExpression(strings),
        ...expressions,
      ]),
      call,
    )
  ...
  runMacroFromCallExpression(
    ...
    if (ts.isPropertyAccessExpression(name)) {
      if (!isMacroIdent(name.name)) {
        return
      }
      const symofArg = resolveAliasedSymbol(
        this.checker,
        this.checker.getSymbolAtLocation(name.expression),
      )
      if (symofArg && hasBit(symofArg.flags, ts.SymbolFlags.Namespace)) {
      ...
      if (!possibleMacros.length) {
        throw new MacroError(
          call,
          `No possible candidates for "${name.name.getText()}" call`,
        )
      } else if (possibleMacros.length > 1) {
        throw new MacroError(
          call,
          `More than one possible candidate for "${name.name.getText()}" call`,
        )
      } else {
        macro = possibleMacros[0]
      }
      const newArgs = ts.factory.createNodeArray([
        this.expectExpression(name.expression),
        ...call.arguments,
      ])
      normalArgs = this.macroStack.length
        ? ts.factory.createNodeArray(
            newArgs.map((arg) => this.expectExpression(arg)),
          )
        : newArgs
    } else if (ts.isIdentifier(name)) {
      if (!isMacroIdent(name)) {
        return
      }
      const nativeMacro = nativeMacros[name.text]
      if (nativeMacro) {
        const macroResult = nativeMacro.call(
          nativeMacro.preserveParams
            ? args
            : ts.factory.createNodeArray(
                args.map((arg) => this.expectExpression(arg)),
              ),
          this,
          call,
        )
        if (!macroResult) {
          ...
          return macroResult as Array<ts.Statement>
        } else if (ts.isStatement(macroResult as ts.Node)) {
          return [macroResult as ts.Statement]
        }
        return [
          ts.factory.createExpressionStatement(macroResult as ts.Expression),
        ]
      }
      macro = this.macros.get(
        resolveAliasedSymbol(
          this.checker,
          this.checker.getSymbolAtLocation(name),
        )!,
    ...
    if (!macro || !macro.body) {
      const calledSym = resolveAliasedSymbol(
        this.checker,
        this.checker.getSymbolAtLocation(name),
      )
      if (
        calledSym?.declarations?.length &&
    ...
    return this.execMacro(
      macro,
      normalArgs || ts.factory.createNodeArray(),
      call,
      target,
    )
  }
  ...
  execMacro(
    ...
    for (let i = 0; i < macro.params.length; i++) {
      ...
      if (param.marker === MacroParamMarkers.Save) {
        const value = param.spread
          ? ts.factory.createArrayLiteralExpression(
              args.slice(param.start) as ts.Expression[],
            )
          : ((args[param.start] || param.defaultVal) as ts.Expression)
    ...
    if (pre.length) {
      this.escapeStatement(
        ts.factory.createVariableStatement(
          undefined,
          ts.factory.createVariableDeclarationList(pre, ts.NodeFlags.Let),
  ...
  expandMacroResults(
    ...
    if (parent) {
      ...
      if (
        prepared.length &&
        ts.isReturnStatement(prepared[prepared.length - 1]) &&
      ...
      return ts.factory.createCallExpression(
        ts.factory.createParenthesizedExpression(
          ts.factory.createArrowFunction(
            ...
            undefined,
            undefined,
            ts.factory.createBlock([...statements, last], true),
          ),
        ),
        undefined,
        undefined,
  ...
  makeHygienic(statements: ts.Statement[]): ts.Statement[] {
    ...
    const makeBindingElementHygienic = (
      name: ts.BindingName,
    ): ts.BindingName => {
          ...
          name.elements.map((el) =>
            ts.isBindingElement(el)
              ? ts.factory.createBindingElement(
                  el.dotDotDotToken,
                  el.propertyName,
                  makeBindingElementHygienic(el.name),
                  el.initializer
                    ? this.expectExpression(el.initializer)
                    : undefined,
                )
              : el,
          ),
    ...
    const visitor = (node: ts.Node): ts.Node => {
      if (ts.isVariableDeclaration(node) && node.pos !== -1) {
        return ts.factory.updateVariableDeclaration(
          node,
          makeBindingElementHygienic(node.name),
          undefined,
          undefined,
          node.initializer
            ? (visitor(
                this.expectExpression(node.initializer),
              ) as ts.Expression)
            : undefined,
        )
      } else if (ts.isIdentifier(node)) {
  ...
  getMarker(param: ts.ParameterDeclaration): MacroParamMarkers {
    if (!param.type) {
      return MacroParamMarkers.None
    }
    const type = this.checker
      .getTypeAtLocation(param.type)
      .getProperty("__marker")
    if (!type) {
      return MacroParamMarkers.None
    }
    const typeOfMarker = (
      this.checker.getTypeOfSymbol(type) as ts.Type
    ).getNonNullableType()
    ...
    switch (typeOfMarker.value) {
      case "Accumulator":
        return MacroParamMarkers.Accumulator
      case "Save":
        return MacroParamMarkers.Save
      default:
        return MacroParamMarkers.None
    }
  }
  ...
  getLiteralFromNode(
    ...
    if (ts.isParenthesizedExpression(node)) {
      ...
      for (const span of node.templateSpans) {
        const lit = this.getLiteralFromNode(
          this.expectExpression(span.expression),
        )
      ...
      for (const prop of node.properties) {
        ...
        obj[name] = this.getLiteralFromNode(
          prop.initializer,
          handleIdents,
          handleTemplates,
          handleObjects,
  ...
  resolveTypeArgumentOfCall(
    ...
    const type = this.checker.getTypeAtLocation(
      macroCall.typeArguments[typeIndex],
    )
    ...
    if (type.isTypeParameter()) {
      if (lastRep && lastRep.indexTypes) {
        const resolvedIndexTypeParameterIndex = lastRep.indexTypes.findIndex(
          (indType) => indType === type,
        )
        if (resolvedIndexTypeParameterIndex !== -1) {
          const resolvedTypeParam =
            lastRep.elementSlices[resolvedIndexTypeParameterIndex]?.[
              lastRep.index
            ]
      ...
      const resolvedTypeParameterIndex =
        lastMacroCall.macro.typeParams.findIndex(
          (arg) => this.checker.getTypeAtLocation(arg) === type,
        )
      if (
        resolvedTypeParameterIndex !== -1 &&
        lastMacroCall.call &&
        ts.isCallExpression(lastMacroCall.call)
      ) {
        const resolvedTypeParam =
          lastMacroCall.call.typeArguments?.[resolvedTypeParameterIndex]
        if (!resolvedTypeParam) {
          return resolveTypeArguments(this.checker, lastMacroCall.call)[
            resolvedTypeParameterIndex
          ]
        }
        return getTypeAtLocation(this.checker, resolvedTypeParam)
      }
    } else {
      const allParams = lastMacroCall.macro.typeParams.map((p) =>
        this.checker.getTypeAtLocation(p),
      )
      const replacementTypes = resolveTypeArguments(
        this.checker,
        lastMacroCall.call as ts.CallExpression,
      )
      if (lastRep && lastRep.indexTypes) {
        ...
        for (let i = 0; i < lastRep.repeatNames.length; i++) {
          ...
          if (replacementType) {
            replacementTypes.push(
              getTypeAtLocation(this.checker, replacementType),
            )
          }
        }
      }
  ...
  findMacroByTypeParams(
    ...
    mainLoop: for (const [sym, macro] of this.macros) {
      ...
      const fnType = this.checker
        .getTypeOfSymbolAtLocation(sym, sym.valueDeclaration!)
        .getCallSignatures()[0]
      const fnTypeParams = macro.typeParams.map((p) =>
        this.checker.getTypeAtLocation(p),
      )
      const anyArray = fnTypeParams.map(
        (p) => p.getConstraint() || this.checker.getAnyType(),
      )
      const fnArgs = fnTypeParams.length
        ? fnType.parameters.map((p) =>
            resolveTypeWithTypeParams(
      ...
      if (this.checker.isTypeAssignableTo(firstType, firstArg)) {
        ...
        for (let i = 0; i < fnArgs.length; i++) {
          ...
          if (!restTypes[i]) {
            if (
              fnArgs[i].getDefault() ||
              fnArgs[i] !== fnArgs[i].getNonNullableType()
            ) {
              continue
            } else {
              continue mainLoop
        ...
        if (restTypes.length > fnArgs.length) {
          ...
          let argType = this.checker.getTypeArguments(
            fnArgs[fnArgs.length - 1] as ts.TypeReference,
          )[0]
  ...
  findMacroByName(node: ts.Node, name: string): Macro | undefined {
    ...
    if (foundMacros.length > 1) {
      throw new MacroError(
        node,
        `More than one macro with the name ${name} exists.`,
      )
    }
  ...
  escapeStatement(...statements: Array<ts.Statement>): void {
    this.escapedStatements[this.escapedStatements.length - 1].push(
      ...statements,
    )
  }
  ...
  addComptimeSignature(
    sym: ts.Node,
    fn: string,
    args: Array<string>,
  ): ComptimeFunction {
    if (this.comptimeSignatures.has(sym)) {
      return this.comptimeSignatures.get(sym) as ComptimeFunction
    }
    const comptime = new Function(...args, fn) as (
      ...args: Array<unknown>
    ) => void
  ...
  strToAST(str: string): ts.NodeArray<ts.Statement> {
    ...
    const uniquelize: (node: ts.Node) => ts.Node = (node: ts.Node) =>
      ts.factory.cloneNode(ts.visitEachChild(node, uniquelize, this.context))
  ...
  cleanupMacros(macro: Macro, extra?: (old: Macro) => void): void {
    for (const [oldSym, oldMacro] of this.macros) {
      if (
        macro.name === oldMacro.name &&
        macro.node.getSourceFile().fileName ===
          oldMacro.node.getSourceFile().fileName &&
        macro.namespace === oldMacro.namespace
      ) {
...
const separators: Record<
  string,
  (transformer: MacroTransformer, body: Array<ts.Node>) => ts.Expression
> = {
  "[]": (_transformer, body) =>
    ts.factory.createArrayLiteralExpression(
      body.map((m) =>
  ...
  "+": (transformer, body) =>
    toBinaryExp(transformer, body, ts.SyntaxKind.PlusToken),
  "-": (transformer, body) =>
    toBinaryExp(transformer, body, ts.SyntaxKind.MinusToken),
  "*": (transformer, body) =>
    toBinaryExp(transformer, body, ts.SyntaxKind.AsteriskToken),
  "||": (transformer, body) =>
    toBinaryExp(transformer, body, ts.SyntaxKind.BarBarToken),
  "&&": (transformer, body) =>
    toBinaryExp(transformer, body, ts.SyntaxKind.AmpersandAmpersandToken),
  "()": (transformer, body) =>
    ts.factory.createParenthesizedExpression(
      toBinaryExp(transformer, body, ts.SyntaxKind.CommaToken),
    ),
  ".": (_, body) => {
  ...
  "{}": (transformer, body) => {
    return ts.factory.createObjectLiteralExpression(
      body
        .filter((el) => ts.isArrayLiteralExpression(el))
        .map((el) => {
          ...
          const string = transformer.getStringFromNode(
            arr.elements[0],
            false,
            true,
          )
