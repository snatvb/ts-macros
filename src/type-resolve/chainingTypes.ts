...
function resolveTypeName(
  ...
  if (hasBit(type.flags, ts.TypeFlags.String)) {
    return "String"
  } else if (hasBit(type.flags, ts.TypeFlags.Number)) {
    return "Number"
  } else if (hasBit(type.flags, ts.TypeFlags.Boolean)) {
    return "Boolean"
  }
...
export function generateChainingTypings(
  checker: ts.TypeChecker,
  ...
  for (const [, macro] of macros) {
    ...
    const decl = ts.factory.createMethodSignature(
      [],
      macro.name,
      macro.node.questionToken,
      macro.node.typeParameters,
      macro.node.parameters.slice(1),
      macro.node.type || UNKNOWN_TOKEN,
    )
  ...
  for (const [type, chainFunctions] of ambientDecls) {
    ...
    decls.push(
      ts.factory.createInterfaceDeclaration(
        ...
        typeName,
        // @ts-expect-error Err
        type.target?.typeParameters?.map((p: ts.TypeParameter) =>
