...
export function patchCompilerHost(
  ...
  const compilerHost =
    host ||
    instance.createCompilerHost(
      config || instance.getDefaultCompilerOptions(),
      true,
    )
...
export function extractGeneratedTypes(
  ...
  return {
    ...
    print: (statements: ts.Statement[]) =>
      printAsTS(printer, statements, parsedSourceFile),
  }
}
...
export default function (
  ...
  const transformer = new MacroTransformer(
    instance.nullTransformationContext,
    program.getTypeChecker(),
    macros,
    { ...(options as TsMacrosConfig), keepImports: true },
    {
      beforeRegisterMacro: (transformer, _sym, macro) =>
        transformer.cleanupMacros(macro),
    },
  )
  ...
  for (let i = 0; i < sourceFiles.length; i++) {
    ...
    if (isTSC)
      ...
      const newNodesSource = instance.createSourceFile(
        sourceFile.fileName,
        sourceFile.text + "\n" + newNodesOnly,
        sourceFile.languageVersion,
        true,
