...
export function getRepetitionParams(
  checker: ts.TypeChecker,
  rep: ts.ArrayLiteralExpression,
): RepetitionData {
...
export function genDiagnosticFromMacroError(
...
export function createObjectLiteral(
  record: Record<string, ts.Expression | ts.Statement | undefined>,
        ...
        obj
          ? ts.isStatement(obj)
            ? ts.factory.createArrowFunction(
                ...
                undefined,
                undefined,
                ts.isBlock(obj) ? obj : ts.factory.createBlock([obj]),
              )
            : obj
    ...
    for (const key in primitive as Record<string, unknown>) {
      assignments.push(
        ts.factory.createPropertyAssignment(
          ...
          primitiveToNode((primitive as Record<string, unknown>)[key]),
        ),
      )
    }
    return ts.factory.createObjectLiteralExpression(assignments)
...
export function fnBodyToString(
  checker: ts.TypeChecker,
  ...
  const visitor = (node: ts.Node) => {
    if (ts.isCallExpression(node)) {
      ...
      if (
        ...
        const name = signature.declaration.name
          ? signature.declaration.name.text
  ...
  return (
    code +
    ts.transpile((fn.body.original || fn.body).getText(), compilerOptions)
  )
}

export function tryRun(
  contentStartNode: ts.Node,
...
export function macroParamsToArray<T>(
...
export function resolveTypeWithTypeParams(
  providedType: ts.Type,
  typeParams: ts.TypeParameter[],
  replacementTypes: ts.Type[],
): ts.Type {
  const checker = providedType.checker
  // Access type
  if ("indexType" in providedType && "objectType" in providedType) {
    const indexType = resolveTypeWithTypeParams(
      (providedType as any).indexType as ts.Type,
      typeParams,
      replacementTypes,
    )
    const objectType = resolveTypeWithTypeParams(
      (providedType as any).objectType as ts.Type,
      typeParams,
      replacementTypes,
    )
  ...
  // Conditional type
    ...
    "checkType" in providedType &&
    "extendsType" in providedType &&
    "resolvedTrueType" in providedType &&
    "resolvedFalseType" in providedType
  ) {
    const checkType = resolveTypeWithTypeParams(
      (providedType as any).checkType as ts.Type,
      typeParams,
      replacementTypes,
    )
    const extendsType = resolveTypeWithTypeParams(
      (providedType as any).extendsType as ts.Type,
      typeParams,
      replacementTypes,
    )
    const trueType = resolveTypeWithTypeParams(
      (providedType as any).resolvedTrueType as ts.Type,
      typeParams,
      replacementTypes,
    )
    const falseType = resolveTypeWithTypeParams(
      (providedType as any).resolvedFalseType as ts.Type,
      typeParams,
      replacementTypes,
    )
    ...
    newType.types = newType.types.map((t) =>
  ...
  //@ts-expect-error Private API
    ...
    callSignature.resolvedReturnType = resolveTypeWithTypeParams(
      originalCallSignature.getReturnType(),
      typeParams,
      replacementTypes,
    )
    callSignature.parameters = callSignature.parameters.map((p) => {
      ...
      newParam.type = resolveTypeWithTypeParams(
        checker.getTypeAtLocation(
          (p.valueDeclaration as ts.ParameterDeclaration).type as ts.Node,
        ),
        typeParams,
        replacementTypes,
      )
 ...
 * of statements.
 */
export function deExpandMacroResults(
  nodes: Array<ts.Statement>,
  ...
  if (ts.isReturnStatement(lastNode)) {
    ...
    if (
      ...
      ts.isArrowFunction(expression.expression.expression)
    ) {
...
export function normalizeFunctionNode(
  checker: ts.TypeChecker,
...
export function getGeneralType(
  checker: ts.TypeChecker,
  type: ts.Type,
): ts.Type {
