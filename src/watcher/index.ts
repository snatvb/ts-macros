...
export function createMacroTransformerWatcher(
  configFileName: string,
  actions: MacroTransformerWatcherActions,
  jsOut?: boolean,
  transformerConfig?: TsMacrosConfig,
  inPrinter?: ts.Printer,
  ...
  const printer = inPrinter || ts.createPrinter(),
    ...
    transformer = new MacroTransformer(
      ...
      transformerConfig,
      {
        beforeRegisterMacro(transformer, _symbol, macro) {
          transformer.cleanupMacros(macro, (oldMacro) =>
  ...
  host.createProgram = (rootNames, options, host, oldProgram) => {
    ...
    const newProgram = oldCreateProgram(
      rootNames,
      options,
      host,
      oldProgram,
    ...
    for (const source of newProgram.getProgram().getSourceFiles()) {
      ...
      if (!oldSource || oldSource.version !== source.version || isForced) {
        ...
        if (typeof transpiled === "string") {
          forcedFilesToGetTranspiled.push(
            ...getFilesThatNeedChanges(source.fileName),
          )
          actions.updateFile(
            source.fileName,
            jsOut
              ? ts.transpile(transpiled, newProgram.getCompilerOptions())
              : transpiled,
            isForced
              ? FileUpdateCause.MacroChange
