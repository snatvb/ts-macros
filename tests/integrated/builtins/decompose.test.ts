...
describe("$$decompose", () => {
  function $stringify(value: any): string {
    ...
    if ($$kindof!(value) === ts.SyntaxKind.PropertyAccessExpression)
      return $stringify!($decomposed[0]) + "." + $stringify!($decomposed[1])
    else if ($$kindof!(value) === ts.SyntaxKind.CallExpression)
      return (
        $stringify!($decomposed[0]) +
        "(" +
        (+[
          "+",
          [$$slice!($decomposed, 1)],
  ...
  it("To stringify the expression", () => {
    ...
    expect($stringify!(console.log(1, true, console.log("Hello")))).to.be.equal(
      'console.log(1, true, console.log("Hello"))',
    )
  })
})
