...
describe("$$propsOfType", () => {
  ...
  it("To return the properties", () => {
    expect($$propsOfType!<{ a: string; b: number }>()).to.be.deep.equal([
      "a",
      "b",
    ])
    expect(
      $test!<{ a: number; __b: string }>({ a: 123, __b: "Hello" }),
  ...
  function $test2<K extends keyof Complex, T extends keyof Complex[K]>(
    key1: K,
    key2: T,
    element: number = 0,
  ) {
